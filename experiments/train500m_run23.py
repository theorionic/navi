"""run23: run22f continued with the anti-collapse fix stack + depth growth.

Base: the completed 19999-step ckpt (4 mem blocks c=512, 8 layers) is
TRANSPLANTED 1:1 into a 16-layer net (memory_every=2 -> 8 mem blocks):
old layer i -> new layer i (even: attn+PKM, odd: attn+FFN), new layers
8-15 fresh (Net2Net depth-growth warm start - zero of the 20k-step trunk
is re-learned).

Fixes on top (from run22f post-mortem: pool alive 0.9-3.5% = side-grid
ceiling 1.56%):
  1. side_top 64 -> 128 (chunked per class in pkm.py, exact) -> reach 6.2%
  2. lb (router-entropy) wired into the loss, lb_weight 0.01
  3. mem-grad x10 -> x30 (gn(mem) was 60x below gn(core))
  4. score_temp anneal 4.0 -> 2.0 over the last 20%
All 537M pool values stay live; no param surgery.

Env: NAVI_STEPS (12000), NAVI_WARMUP (500), NAVI_RESUME=0 (warm-start
from RUN22_CKPT, not resume), NAVI_KEEP_CKPT (2).
"""
import sys
...
TAG = "bpe23"
STEPS = int(os.environ.get("NAVI_STEPS", "12000"))
...
TEMP_END = float(os.environ.get("NAVI_TEMP_END", "2.0"))
CKPT_DIR = "/kaggle/working/experiments"
CKPT_PREFIX = "ckpt_bpe500m23_step"
GEN_ON = os.environ.get("NAVI_GEN", "1") == "1"
GEN_LEN = 128
VOCAB = 16384
RUN22_CKPT = os.environ.get(
    "NAVI_RUN22_CKPT",
...
def temp_at(step_i):
    # 4.0 -> TEMP_END over the last 20% of training: sharpen reads as keys
    # lock in; early training keeps the soft read for gradient spread.
    frac = step_i / max(1, STEPS - 1)
    if frac < 0.8:
        return 4.0
    f = (frac - 0.8) / 0.2
    return 4.0 + f * (TEMP_END - 4.0)


def loss_fn(model, p, ids, tg, temp):
    # model is the return_aux=True instance (Navi(cfg_m, mem_cfg,
    # return_aux=True)); logits, aux, lb_total triple.
...
def do_generation(model, params, tok, step_i):
    ...
    texts = generate(model_eval, params, tok, PROMPTS, GEN_LEN, temp=0.8,
                     seed=step_i + 42)
...
def main():
    ...
    mem_cfg = MemoryConfig(c1=512, c2=512, cand_k=CAND_K, side_top=SIDE_TOP,
                           ...
                           lb_weight=LB_WEIGHT)
    cfg_m = ModelConfig(d_model=512, n_layers=16, n_heads=8, memory_every=2,
                        vocab_size=VOCAB)
    model = Navi(cfg_m, mem_cfg, return_aux=True)  # triple: (logits, aux, lb)
    model_eval = Navi(cfg_m, mem_cfg)  # plain logits path for val/gen
    ...
    # ---- warm-start transplant from the completed run22f ckpt ----
    print(f"[{TAG}] transplanting {RUN22_CKPT}", flush=True)
    with open(RUN22_CKPT, "rb") as f:
        st22 = pickle.load(f)
    old = st22["params"]
    # old block i -> new block i for i in 0..7 (layout identical: even
    ...
    new_p = p0.get("params", p0)
    got = new_p.get("block_0", {}).get("mem", {}).get("k1")
    same = bool(jnp.array_equal(check, got)) if check is not None else False
    print(f"[{TAG}] transplant block_0/mem/k1 carried: {same}", flush=True)
    ...
    for i in range(start, STEPS):
        ...
        if i % GEN_EVERY == 0 or i == STEPS - 1:
            v = val_loss(model_eval, p, val)
            val_hist.append((i, v))
            print(f"[{TAG}] VAL@{i} bpc {v:.4f} (held-out)", flush=True)
            cov = pool_coverage(model, p, val)
