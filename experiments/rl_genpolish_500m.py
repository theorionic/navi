...
REF_W = float(os.environ.get("NAVI_REF_W", "0.3"))  # ref-LM bonus weight
if REF_W <= 0.0:                                   # pure lexical mode
    _QR = quality_reward
    quality_reward = lambda tails, ref_logp=None: _QR(tails)
...
def quality_reward(tails, ref_logp=None):
    ...
    ref_logp: optional (N,) mean per-byte log-prob of tails under the
    frozen pretrained reference LM (natural units). Mapped to a [0,1]
    bonus via sigmoid and blended in; shapes toward the SFT distribution
    instead of pure lexical rules."""
    low = jnp.where((tails >= 65) & (tails <= 90), tails + 32, tails)
    letter_ok = (((low >= 97) & (low <= 122)) | (low == 32) |
                 (low == 10)).mean(axis=-1)
    w_ok, rep_pen = word_ok_and_rep(tails)
    nr = jnp.minimum(norepeat4(tails), rep_pen)
    lex = 0.15 * letter_ok + 0.40 * w_ok + \
        0.45 * nr * (0.3 + 0.7 * w_ok)
    if ref_logp is None:
        return lex
    # ref LM bonus: exp(mean logp) is the per-byte prob; sigmoid(x - ln 0.5)

    ref_bonus = jax.nn.sigmoid(ref_logp - jnp.log(0.5))
    return (1.0 - REF_W) * lex + REF_W * ref_bonus
...
def main():
    ...
    # lexical-only reward for the headline number (same scale across
    # runs regardless of REF_W); ref-shaped reward used inside RL only.
    ...
    def polish_chunk_jit(pp, pp0, prompts, kk):
        ...
        tails = jnp.transpose(outs, (0, 2, 1)).reshape(-1, GEN_L)

        # frozen reference LM logp on the generated tails (pretrained
        # weights pp0, no grad): shapes reward toward the pretrain
        # distribution instead of lexical rules only.
        def ref_ll(w):
            logits = model.apply(w, tails[:, :-1], train=False)
            logp = jax.nn.log_softmax(logits)
            lp = jnp.take_along_axis(
                logp, tails[:, 1:][..., None], axis=-1)[..., 0]
            return lp.mean(axis=-1)                          # (N,)
        ref_logp = jax.lax.stop_gradient(ref_ll(pp0))
        r = quality_reward(tails, ref_logp)
        adv = r - r.mean()
    ...
    # lexical-only headline (same scale as BEFORE); report ref-LM
    # per-byte logp too: if RL helped, logp should NOT have collapsed.
    @jax.jit
    def ref_logp_np(tails):
        logits = model.apply(pp0, tails[:, :-1], train=False)
        logp = jax.nn.log_softmax(logits)
        lp = jnp.take_along_axis(
            logp, tails[:, 1:][..., None], axis=-1)[..., 0]
    ...
    tails_all = jnp.asarray(
        np.concatenate([o.reshape(-1, GEN_L) for o in outs_np2]), jnp.int32)
    log(f"AFTER: mean quality reward {r1.mean():.3f} (before {r0.mean():.3f}) "
        f"ref logp/byte {float(ref_logp_np(tails_all).mean()):.3f}")
    ob2 = outs_np2
    for i in range(3):
