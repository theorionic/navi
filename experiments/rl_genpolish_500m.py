...
from fineweb_data import FineWebFeed, BOS, EOS
...
def main():
    ...
    def make_prompts(n, seed):
        ...
        for _ in range(n):
            ...
            if sp > 8:
                chunk = chunk[:sp]
            pr.append([BOS] + list(chunk))
        # pad right with EOS to same length, remember lengths
        maxl = max(len(x) for x in pr)
        arr = np.full((n, maxl), EOS, dtype=np.int32)
        lens = np.zeros(n, dtype=np.int32)
        for i, x in enumerate(pr):
            arr[i, :len(x)] = x
            lens[i] = len(x)
        return jnp.asarray(arr, jnp.int32), jnp.asarray(lens, jnp.int32)

    # ------------------------------- eval: greedy samples + reward -------
    eval_prompts, eval_lens = make_prompts(EVAL_TESTS, 777)

    @jax.jit
    def sample_batch(pp, prompts, lens, key):
        """(N_PROMPTS, N_SAMPLES, SEQ) windows; generated bytes are scan
        outputs. Prompt sits at [0:len]; buffer slots AFTER len are
        invisible via causal mask until filled."""
        P = prompts.shape[1]
        def one_prompt(pr, ln, k1):
            ...
            buf0 = buf0.at[:, :P].set(pf)
            buf0 = buf0.at[:, P].set(BOS)
            # start writing at position P (after BOS)
            (buf, _), out = jax.lax.scan(
                gstep, (buf0, k1),
                jnp.arange(P + 1, P + 1 + GEN_L) - 1)
            return buf, out  # buf (S, SEQ), out (GEN_L, S)
        keys = jax.random.split(key, prompts.shape[0])
        bufs, outs = jax.vmap(one_prompt)(prompts, lens, keys)
        return bufs, outs   # bufs (P, S, SEQ), outs (P, GEN_L, S)

    # reward per sample
    def batch_reward(outs):
        """outs (P, GEN_L, S) -> rewards (P, S)"""
        r = jax.vmap(jax.vmap(quality_reward, in_axes=1), in_axes=0)(outs)
        return r  # (P, S)

    # quick pre-check: sample from pretrained model, measure reward
    kk = jax.random.PRNGKey(42)
    pp = shard_tree(p)
    bufs, outs = sample_batch(pp, eval_prompts, eval_lens, kk)
    r0 = np.asarray(batch_reward(outs))
    log(f"BEFORE: mean quality reward {r0.mean():.3f} "
        f"(ascii/word/loop mix; tails below)")
    ob = np.asarray(outs).transpose(0, 2, 1)  # (P, S, L)
    for i in range(3):
    ...
    @jax.jit
    def polish_round(pp, oo, prompts, lens, kk):
        def one_prompt(pr, ln, k1):
            pf = jnp.tile(pr, (N_SAMPLES, 1))
            ...
            buf0 = jnp.zeros((N_SAMPLES, SEQ), jnp.int32)
            buf0 = buf0.at[:, :pr.shape[0]].set(pf)
            buf0 = buf0.at[:, pr.shape[0]].set(BOS)
            (buf, _), out = jax.lax.scan(
                gstep, (buf0, k1),
                jnp.arange(pr.shape[0] + 1, pr.shape[0] + 1 + GEN_L) - 1)
            return buf, out

        keys = jax.random.split(kk, prompts.shape[0])
        bufs, outs = jax.vmap(one_prompt)(prompts, lens, keys)
        r = batch_reward(outs)                       # (P, S)
        adv = r - r.mean()                            # batch baseline
        # log-prob of generated bytes under CURRENT params; gradient
        # ascends adv-weighted logp of the sampled byte sequences.
        # flatten (P, S, SEQ) -> (P*S, SEQ); targets = buf shifted left
        P_ = prompts.shape[1]
        bufs_f = bufs.reshape((-1, bufs.shape[-1]))

        def lp_of(w):
            logits = model.apply(w, bufs_f, train=True)
            logp = jax.nn.log_softmax(logits)
            lp = jnp.take_along_axis(
                logp, bufs_f[:, 1:][..., None], axis=-1)[..., 0]
            T = bufs_f.shape[1] - 1
            P_ = prompts.shape[1]
            mask = (jnp.arange(T) >= P_).astype(lp.dtype)
            return (lp * mask[None, :]).sum(-1)          # (P*S,)

        adv_f = adv.reshape(-1)

        def loss_fn(w):
            lp = lp_of(w)
            pg = -(lp * adv_f).sum() / jnp.maximum(1.0, jnp.abs(adv_f).sum())
            return pg
    ...
    for rnd in range(ROUNDS):
        # fresh prompts each round from the val buffer
        if rnd % 4 == 0:
            pr_arr, ln_arr = make_prompts(N_PROMPTS, 1000 + rnd)
        kk, sub = jax.random.split(kk)
        pp, oo, rm = polish_round(pp, oo, pr_arr, ln_arr, sub)
        hist.append(float(rm))
        if rnd % 20 == 0 or rnd == ROUNDS - 1:
            log(f"  polish {rnd:3d}: batch reward {rm:.3f}")

    # ------------------------------- post eval ---------------------------
    bufs2, outs2 = sample_batch(pp, eval_prompts, eval_lens, kk)
    r1 = np.asarray(batch_reward(outs2))
    log(f"AFTER: mean quality reward {r1.mean():.3f} (before {r0.mean():.3f})")
    ob2 = np.asarray(outs2).transpose(0, 2, 1)
    for i in range(3):
